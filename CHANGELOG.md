...
- **결과 대시보드 구조 전역 표준화 ([대형 메인 카드 + 3단 서브 요약 카드])**:
  - `LoanSummaryCards`: 대출이자 결과 대시보드를 타 계산기(연복리, 연봉, 목표자산)와 100% 동일한 위계로 개편.
    - 최상단 대형 메인 카드: 첫 달 상환액(1회차) 대형 타이포(`tabular-nums`) + 일렉트릭 라임 강조 + 한글 금액 병기 + 원클릭 결과 복사 버튼(`handleCopy`).
    - 3단 서브 요약 카드: `총 상환금액`, `총 대출이자`(원금 대비 이자율 뱃지), `대출 원금`으로 표준화.
- **인풋 vs 슬라이더 단일화 원칙 적용 (중복 배제)**:
  - 금액/돈은 직접 숫자 입력창(`NumericInput`)만 사용하고, 비율/기간/신체치수는 단독 풀 와이드 `Slider` + 상단 실시간 수치 표기 + 하단 프리셋 칩으로 단일화.
  - `CalculatorForm`: 기간 및 수익률 인풋 필드 제거 후 단독 슬라이더로 단일화, 수익률 슬라이더 스텝 `0.5%`로 통일.
  - `BmiForm`: 신장 및 체중 인풋 필드와 `-1/+1` 스텝 버튼을 제거하고, 풀 와이드 단독 슬라이더 + 상단 수치 표기 + 대표 프리셋 칩으로 일괄 개편.
- **탭(Tab) vs 라디오(Radio) 버튼 UI 분리 원칙 적용**:
  - 화면/뷰 전환 목적의 탭은 랩핑 박스(`SegmentedControl` 또는 `Tabs`)를 유지.
  - 폼 내부 옵션 선택 라디오 그룹은 외곽 박스 랩핑을 해제하고 시원한 독립 선택 칩/버튼(`SelectableChip`) 그룹으로 변경:
    - `CalculatorForm`: 적립 주기 선택(매월, 매년, 적립 없음) 독립 칩 그룹화.
    - `BmiForm`: 성별 선택(남성, 여성) 독립 칩 그룹화.
    - `LoanForm`: 상환 방식(원리금균등, 원금균등, 만기일시) 독립 칩 그룹화.
    - `SalaryForm`: 급여 형태(연봉, 월급) 및 퇴직금 지급 방식(별도, 포함) 독립 칩 그룹화.
    - `GoalForm`: 과세 방식(일반과세, ISA 절세, 비과세) 독립 칩 그룹화.
- **네비게이션 및 홈 대시보드 출시 예정(coming-soon) 정책 최적화**:
  - `HomeApp`: 계산기 목록 정렬 시 즉시 이용 가능한 도구(`active`) 우선 배치, 출시 예정(`coming-soon`) 카드는 목록 최하단으로 정렬.
  - `SidebarDrawer`: 좌측 사이드바/드로어 메뉴 목록에서 출시 예정(`coming-soon`) 계산기를 필터링하여 완전히 제외(즉각 실행성 강화).
- **결과 대시보드 메인 복사 버튼 및 3단 서브카드 규격 일원화**:
  - `SummaryCards` (연복리): 메인 결과 카드(세후 최종 수령액) 우측 상단에 원클릭 결과 복사 버튼(`handleCopy`, `Copy`/`Check` 피드백) 추가.
  - `GoalSummaryCards` (목표역산): 메인 결과 카드에 원클릭 결과 복사 버튼 추가 및 3단 서브 요약 카드를 PRD 3.7 명세(`총 투입 원금`, `예상 복리 수익`, `이자/수익 기여도`)에 맞춰 단일 규격으로 개편.
- **단위 변환기 폼 및 결과 카드 높이 100% 일원화 (`UnitConverterApp`)**:
  - `DualConverterCard`: flex 높이 균등 배치(`items-stretch`, `h-full flex flex-col justify-between`) 및 단위 공식 설명 최소 높이 확보로 From/To 카드 높이 완전 일치.
  - `unitConverter.ts`: 7개 전 카테고리 40여 개 단위 전체에 누락되었던 공식 설명(`description`)을 일괄 보강하여 항상 3단 구조 유지.
  - `MultiResultGrid`: 전체 서브 카드 설명란 최소 높이 지정으로 모든 카드 높이 균일화.
- **연복리 과세 방식 입력 UI 표준화 (`CalculatorForm`)**:
  - 구형 `<Select>` 드롭다운을 독립 3단 선택 칩 그룹(`SelectableChip`, 3분할 대칭 그리드) 및 과세 방식 실시간 친절 안내 가이드 박스로 일원화.
  - `TaxType` 데이터 모델에서 미사용 `'custom'` 잔재 정리 및 이전 링크 안전 폴백 처리.
- **코드 품질 및 Fowler 스멜 개선 (`goalCalculator.ts`)**:
  - 미래가치(FV) 및 월 적립액(PMT) 중복 수식을 단일 헬퍼 함수 `calculatePMT`로 추출하여 중복 코드 해소.
  - 음수 수익률 분기 일원화 및 상수에 한국어 설명 주석 보강.
- **PRD 및 품질 검증**:
  - `PRD.md` v1.9.32 동기화, 네비게이션 출시 예정 제외 및 메인 최하단 정렬 규칙, 목표역산 안내 가이드(`GoalInfoCard`) 명세 보강.
  - 전체 27개 테스트 스위트, 139개 테스트 100% 통과 (Pass) 및 프로덕션 번들 빌드(`tsc -b && vite build`) 무결점 성공.

## [1.9.31] - 2026-09-16

### 7번째 신규 계산기 모듈: 목표 자산 역산 계산기 출시 (Goal Target Calculator `/goal`)
- **금융 PMT 기반 필요 월 적립액 역산 엔진 구현 (`goalCalculator.ts`)**:
  - 목표 자산(1,000만~100억), 목표 기간(1~40년), 예상 연 수익률(-5~30%), 초기 보유 자금(거치금), 과세 방식(일반 15.4%, ISA 9.9%, 비과세 0%) 입력.
  - 월초 적립 기준 복리 공식 기반 필요 월 적립액, 총 투입 원금, 예상 복리 이자, 연도별 자산 형성 궤적 산출.
  - 3.5%(예적금), 7.0%(시장평균), 10.0%(적극투자) 대표 수익률 시나리오 대조 데이터 자동 생성.
- **결과 대시보드 및 데이터 시각화 컴포넌트 구축 (`GoalApp.tsx` & `components/`)**:
  - `GoalSummaryCards`: 메인 필요 월 적립액 강조 카드 + 3단 서브 요약 카드(총 투입 원금, 예상 복리 수익, 최종 달성액).
...
- **README.md 최신화**:
  - 버전 뱃지 및 최하단 브랜딩 버전 `v1.9.28` 동기화.
  - 지원 계산기 모듈 테이블에 BMI(비만도) 계산기 항목 및 바로가기 링크 반영.
  - 핵심 특장점 섹션에 '전 페이지 일관 모바일 플로팅 공유(FAB)' 설명 추가.
  - 최신 테스트 통과 지표(131 Passed) 뱃지 동기화.
- **PRD.md 목차 및 디렉터리 구조 트리 최신화**:
  - 목차 섹션 3.5(연봉 계산기), 3.6(환율 계산기), 3.7(BMI 비만도 계산기), 4.7(BMI 계산기 테스트 명세) 번호 정합성 일치.
  - 2.4 모듈형 컴포넌트 아키텍처 디렉터리 트리에 `features/bmi/`, `FloatingShareButton.tsx`, `useShare.ts`, `bmiCalculator.ts` 최신 구조 반영.
  - PRD 헤더 버전 `v1.9.28` 동기화.
- **TODO.md 기획 로드맵 최신화**:
  - `2.3 전 페이지 일관 전역 플로팅 공유 버튼(FAB)` 완료 체크(`[x]`) 및 `v1.9.27` 완료 마킹.
  - 6.0 배포 및 출시 섹션에 `v1.9.22`부터 `v1.9.27`까지의 버전별 완료 이력 누락분 보강.
- **사이트 메타데이터 및 설정 동기화**:
  - `src/config/site.ts` description에 'BMI 비만도' 문구 추가 및 버전 `1.9.28` 반영.
  - `package.json` 및 `src/config/site.test.ts` 버전 `1.9.28` 동기화.

...
2. [제품 아키텍처 및 네비게이션 구조](#2-제품-아키텍처-및-네비게이션-구조)
   ...
   - 2.3 전역 UI/UX 및 디자인 시스템 표준 규격 (Global UI/UX & Design Standards)
...
- **미니멀 네비게이션 및 타이틀 원칙 (라벨 전면 배제 및 출시 예정 항목 숨김)**:
  ...
  - **출시 예정(coming-soon) 항목 사이드바 노출 제외**: 사이드바/드로어는 즉시 사용 가능한 계산기로의 빠른 이동(Quick Jump)이 목적이므로, 클릭해도 사용할 수 없는 준비 중인 항목은 메뉴 목록에서 필터링하여 완전히 숨김 처리(`status !== 'coming-soon'`).
...
### 2.3 전역 UI/UX 및 디자인 시스템 표준 규격 (Global UI/UX & Design Standards)
모든 계산기 모듈과 화면 인터페이스는 프로젝트 루트의 `ghost.design.md`를 단일 진실 공급원(SSOT)으로 삼으며, 예외 없이 다음 표준 규격을 전역 일관되게 적용합니다.

#### 2.3.1 Ghost 디자인 시스템 핵심 원칙
- **절제와 집중 (Restraint & Focus)**: 다색상 배치를 엄격히 통제하고, 차분한 모노크롬 베이스(Near-black & Warm White)를 바탕으로 아이브로우 라벨과 핵심 인터랙션에만 절제된 일렉트릭 라임(Electric Lime) 전압을 부여.
- **서체 및 숫자 판독성 (Typography & Tabular Numbers)**:
  - 플랫폼 전반의 모든 텍스트에 **Pretendard Variable** 단일 서체 강제 적용 (타 폰트 혼용 전면 금지).
  - 금액, 환율, 변환 수치 등 모든 금융/측정 데이터에 고정폭 숫자(`tabular-nums`)를 결합하여 수치 변경 시 자릿수 흔들림 차단.
- **전문성과 신뢰감**: UI 레이아웃 및 팁 안내 문구에서 알록달록한 컬러 이모지를 전면 배제하고, 단정한 텍스트 및 Lucide React 표준 라인 아이콘만 사용.

#### 2.3.2 공통 반응형 레이아웃 및 그리드 규격 (Layout & Breakpoints)
- **데스크톱 (>=1024px)**:
  - 좌측 고정 사이드바(256px)를 제외한 메인 뷰포트에서 **12컬럼 그리드(좌측 폼 5 : 우측 결과 대시보드 7)** 레이아웃 통일.
  - 전 계산기가 동일한 분할 비율을 유지하여 페이지 이동 시 시선과 레이아웃 일관성 보장.
- **태블릿 및 모바일 (<1024px)**:
  - 단일 열 수직 스택으로 전환: 상단에 [입력 폼] 배치 ➔ 하단에 [결과 대시보드] 전개.
  - 가로 스크롤 없이 엄지손가락 터치 반경 내에서 입력과 조회가 자연스럽게 이어지도록 최적화.

#### 2.3.3 입력 폼 표준 인터페이스 규격 (Form Standards)
- **폼 헤더 및 표준 초기화 액션**:
  - 폼 카드 헤더 우측 상단에 Ghost 스타일 초기화 버튼 고정 배치 (`RotateCcw` 아이콘 + "초기화" 라벨).
  - 불필요한 확인 팝업 없이 원클릭으로 해당 계산기의 기본값으로 즉시 복원.
  - 좁은 화면에서도 타이틀과 겹치거나 개행되지 않도록 컴팩트한 크기 유지.
- **인풋 vs 슬라이더 단일화 원칙 (중복 배제)**:
  - 인풋과 슬라이더가 동시에 한 화면에 병존하여 세로 길이를 낭비하거나 조작을 분산시키는 현상 원천 배제.
  - **돈(금액)**: 직접 숫자 입력창(`NumericInput`)만 사용 (터치 친화적 대형 규격, 우측 정렬, 볼드 폰트, 3자리 콤마, 우측 내부 단위 심볼 `원`).
  - **비율 / 기간 / 신체 계측치**: 인풋 필드를 제거하고, **풀 와이드 단독 슬라이더(`Slider`) + 상단 실시간 수치 표기 + 하단 프리셋 칩(`SelectableChip`)**으로 일괄 단일화.
  - **슬라이더 스텝 표준화**: 연복리 및 목표자산 등 모든 예상 수익률 슬라이더의 증감 단위(step)를 `0.5%`로 전역 일관화.
- **탭(Tab) vs 라디오(Radio) 버튼 UI 분리 원칙**:
  - **탭 (화면 모드 / 뷰 전환)**: 캡슐형 박스로 랩핑된 형태(`SegmentedControl` 또는 `Tabs`)를 적용하여 뷰 전환 상태를 명확히 표현 (예: 단일 vs 비교 모드 전환, 근로자 vs 사업주 부담금 테이블 전환).
  - **라디오 버튼 (폼 내부 단일 옵션 선택)**: 외곽을 회색 박스로 랩핑하지 않고, **독립된 선택 칩 그룹**(`SelectableChip` 기반)으로 균일한 대칭 그리드(`grid grid-cols-N gap-2`, 2분할/3분할/4분할) 및 **동일한 단일 규격(`h-auto py-2 text-xs sm:text-sm font-semibold justify-center text-center`)**으로 시원하게 나열하여 전 페이지 일관성 및 모바일 터치 조작성 극대화 (예: 성별 선택, 적립 주기 선택, 대출 상환 방식 및 거치 기간 선택, 급여 형태 및 퇴직금 선택, 과세 방식 선택).

#### 2.3.4 결과 대시보드 표준 규격 (Result Dashboard Standards)
- **전 계산기 결과 대시보드 구조 통일 (메인 하이라이트 + 3단 서브 카드)**:
  - 대출이자 계산기를 포함한 모든 계산기 대시보드는 **[1. 메인 결과 카드] + [2. 3단 서브 요약 지표 카드]**의 단일 시각적 계층 구조를 예외 없이 적용.
  - **메인 핵심 결과 카드 (Primary Highlight Card)**:
    - 결과 대시보드 최상단에 배치하여 계산기의 최종 핵심 지표(복리 최종금액, 목표 필요적립액, 대출 첫 달 상환액, 월 실수령액 등)를 대형 타이포(`tabular-nums`)와 일렉트릭 라임 액센트로 강조.
    - 한글 금액 단위(예: `1억 5,000만 원`, `월 234만 원`, `첫 달 124만 원`) 병기.
    - 핵심 결과를 한 번에 클립보드에 담을 수 있는 원클릭 복사 버튼 및 시각적 피드백 제공.
  - **서브 요약 지표 카드 유동 리플로우 (Sub Metric Cards)**:
    - 핵심 지표 하단에 배치되는 3단 보조 지표 카드는 컨테이너 너비에 따라 유동 리플로우(`minmax(185px, 1fr)`) 적용.
    - 넓은 화면에서는 3열, 1024px 데스크톱(우측 7컬럼 환경)에서는 2열로 자연스럽게 리플로우되어 카드가 찌그러지는 현상 방지, 모바일에서는 1열 스택.
- **시각화 차트 (Data Visualizations)**:
  - Recharts 기반 영역/도넛/라인 차트를 결과 대시보드 중앙에 배치.
  - 상단 범례/컨트롤은 가용 폭에 따라 자연스럽게 줄바꿈되도록 구성하여 모바일 잘림 차단.
  - 툴팁 레이어의 우선 쌓임 맥락(z-index)을 확보하여 중앙 텍스트나 다른 요소에 가려지지 않도록 보장.
- **금융 및 생활 상식 안내 카드 (`InfoCard`)**:
  - 모든 계산기 최하단에 공통 `InfoCard` 컴포넌트를 배치하여 핵심 공식, 세무 유의사항, 실전 팁을 일관된 단일 리스트 패널로 제공 (컬러 이모지 전면 배제).
- **상세 표 및 내보내기 규격 (`Table` & `CSV 다운로드`)**:
  - 연도별/월별 상세 흐름표는 전역 테이블 공통 컴포넌트(`ui/table.tsx`)를 SSOT로 적용.
  - 다운로드 버튼 명칭은 **"CSV 다운로드"**로 통일하고, 엑셀 호환 UTF-8 BOM 인코딩 보장.
...
- **주요 구성 및 UX 규격 (모바일 스크롤 최소화 및 헤더 고정 원칙)**:
  1. **글로벌 헤더 일원화 타이틀 원칙 (본문 중복 타이틀 완전 배제)**:
   ...
   2. **모바일 2열 콤팩트 카드 그리드 (Mobile 2-Column Grid) 및 정렬 규칙**:
      - **모바일 화면(뷰포트)에서 한눈에 전체 계산기를 조망하고 스크롤을 획기적으로 줄이기 위해 모바일 기본 2열 그리드(`grid-cols-2 sm:grid-cols-2 lg:grid-cols-3`) 채택**.
      - **카드 정렬 정책 (활성 우선 / 출시 예정 최하단)**: 즉시 사용 가능한 계산기(`status === 'active'`)가 목록 앞쪽에 우선 노출되고, 준비 중인 출시 예정(`status === 'coming-soon'`) 카드는 항상 목록의 최하단(맨 뒤)에 배치되어 사용자의 탐색 효율 및 도구 전환율 극대화.
      - 불필요하게 긴 텍스트 문단과 해시태그 칩을 배제하고, [아이콘 + 계산기 명칭 + 간결한 설명 + 시작 링크] 중심의 미니멀 콤팩트 카드 레이아웃 적용 (태그 칩 완전 삭제로 세로 높이 축소 및 시각적 잡음 배제).
      - 주요 활성 계산기들이 모바일 1~2 스와이프 내에 모두 손쉽게 노출되어 터치 진입성 극대화.
...
- **주요 입력 파라미터**:
  - 초기 원금: 직접 금액 입력 (빠른 프리셋: `+100만`, `+500만`, `+1,000만`, `+5,000만`, `+1억`).
  - 정기 적립액 및 주기: 월납 / 연납 / 미적립 선택 및 납입액 입력.
  - 투자 기간: 1 ~ 40년 (빠른 프리셋: `5년`, `10년`, `20년`, `30년`).
  - 연 예상 수익률: -5% ~ 50% (빠른 프리셋: `-3%`, `3.5%`, `8%`, `15%`).
  - 복리 주기: 월복리(기본), 연복리, 분기복리, 일복리.
  - 과세 방식: 일반과세(15.4%), ISA(9.9%), 비과세(0%).
- **핵심 복리 및 세무 산출 로직**:
  - 월초/연초 적립 기준 복리 증식 공식 적용.
  - 하락장/원금 손실 구간(-5% ~ 0%) 시뮬레이션 지원 (손실 발생 시 소득세 0원 자동 면제).
  - 시나리오 A/B 듀얼 비교 모드: 두 가지 투자 전략 간 최종 자산 및 이자 격차 실시간 대조.
- **결과 대시보드 지표 및 시각화**:
  - **메인 지표**: 세후 최종 수령액, 원금 대비 수익 배수, 한글 금액 병기.
  - **3단 서브 지표**: 총 투자원금, 세후 순이자, 이자소득세.
  - **자산 추이 차트**: 단일 모드 시 누적 영역형 차트(`AreaChart`), 비교 모드 시 멀티 라인 차트(`LineChart`).
  - **연도별 상세 흐름표**: 연차별 납입원금, 당해연도 세전이자, 누적 순이자, 세후 총평가액, 수익률 및 CSV 다운로드 지원.

### 3.2 [생활/측정] 단위 변환기 (`UnitConverterApp` - 구현 완료)
- **기능 요약**: 일상 생활, 부동산 거래, 해외 직구 및 기술 규격에서 자주 쓰이는 단위를 실시간으로 상호 변환하고, 관련 단위 환산 결과를 한눈에 확인하는 계산기.
- **지원 카테고리 및 핵심 단위**:
  - **넓이 (부동산 특화)**: 평(坪), 제곱미터(㎡), 헥타르(ha), 에이커(ac), 평방피트(ft²) 등 (아파트 공급/전용면적 84㎡ ↔ 25.4평 원클릭 프리셋).
  - **길이**: 센티미터(cm), 미터(m), 킬로미터(km), 인치(in), 피트(ft), 야드(yd), 마일(mi), 자/척(尺).
  - **무게**: 그램(g), 킬로그램(kg), 톤(t), 파운드(lb), 온스(oz), 돈(3.75g 순금), 근(600g).
  - **부피**: 밀리리터(mL), 리터(L), 갤런(gal, US 액량 3.785L), 배럴(bbl), 플루이드 온스(fl oz).
  - **온도**: 섭씨(℃), 화씨(℉), 켈빈(K) (비선형 공식: $\text{℉} = \text{℃} \times 1.8 + 32$, $\text{K} = \text{℃} + 273.15$).
- **변환 인터페이스 및 대시보드**:
  - **출발/도착 듀얼 카드 & 맞바꾸기(Swap)**: 직관적인 입력 및 결과 카드 구조와 1터치 맞바꾸기 버튼.
  - **정밀도 설정**: 소수점 0, 2, 4, 6자리 선택기.
  - **전체 단위 일괄 변환 그리드**: 값 입력 즉시 해당 카테고리 내 모든 단위로 환산된 결과를 테이블로 일괄 제공.

### 3.3 [통화/글로벌] 환율 계산기 (`ExchangeApp` - 구현 완료)
- **기능 요약**: 글로벌 주요 6대 통화 간 금액을 매매기준율 및 은행 환전 수수료/우대율(Spread Discount)을 반영하여 즉시 상호 환산하는 계산기.
- **오프라인 지원 및 기준 환율**:
  - 인터넷 단절 상태(PWA 오프라인)에서도 독립 동작하도록 최신 고시 매매기준율 기본 내장.
  - 네트워크 연결 시 최신 환율 비동기 갱신 (5초 타임아웃 안전망 적용).
- **지원 통화**:
  - `KRW`(대한민국 원), `USD`(미국 달러), `JPY`(일본 100엔), `EUR`(유로), `CNY`(중국 위안), `GBP`(영국 파운드).
- **주요 입력 및 환전 옵션**:
  - 환전 거래 방식: 매매기준율, 현찰 살 때, 현찰 팔 때, 송금 보낼 때, 송금 받을 때.
  - 환전 우대율 프리셋: `90%`, `80%`, `50%`, `0%` (수수료 우대).
  - 빠른 금액 프리셋: `USD $100`, `USD $200`, `JPY 10,000엔`, `EUR 100유로`, `KRW 100만원`.
- **결과 대시보드 지표**:
  - **환산 결과 및 우대 혜택**: 최종 환산 금액, 적용 환율, 절약된 수수료(우대 할인액) 표시.
  - **우상단 기준 환율 스마트 표기**: 국내 금융 관행에 맞춰 원화 ➔ 외화 시에도 외화 1단위(엔화 100엔)당 원화 가치로 역산 표기 (예: `1 USD = 1,350.00 KRW`).
  - **전체 통화 일괄 환산 그리드**: 입력 즉시 나머지 5개 통화로 환산된 금액을 한 번에 비교 조회.

### 3.4 [금융/투자] 대출 이자 및 상환방식 비교 계산기 (`LoanApp` - 구현 완료)
- **기능 요약**: 주택담보대출, 전세대출, 신용대출 등 이용 시 3대 상환방식(원리금균등, 원금균등, 만기일시) 간의 총 이자비용 격차와 월별 현금흐름을 한눈에 비교하고 중도상환 혜택을 시뮬레이션하는 계산기.
- **주요 입력 파라미터**:
  - 대출 원금: 직접 금액 입력 (빠른 프리셋: `+1,000만`, `+5,000만`, `+1억`, `+3억`, `+5억`).
  - 연 이자율: 직접 입력 및 슬라이더 (0.1% ~ 20.0%, step 0.1%).
  - 대출 기간: 1년 ~ 50년 (빠른 프리셋: `1년`, `3년`, `5년`, `10년`, `20년`, `30년`, `40년`, `50년`).
  - 거치 기간: 0개월 ~ 대출 기간 미만 (거치 기간 중 이자만 납입).
  - 중도상환 시뮬레이터 (선택 토글): 실행 후 N개월차, 조기상환 원금, 수수료율(기본 1.2%).
- **상환방식별 산출 공식 및 비즈니스 로직**:
  - **원리금균등분할상환**: 매월 동일한 총 상환액(원금+이자) 납부, 마지막 달 단수 차액 0원 정밀 보정.
  - **원금균등분할상환**: 매월 동일한 원금 분할 상환 + 대출 잔액에 따른 월이자 납입 (점진적 상환액 감소).
  - **만기일시상환**: 대출 기간 동안 매월 이자만 납부하고 만기일에 원금 전액 일괄 상환.
  - **중도상환수수료 3년 슬라이딩 감면 공식**:
    - $\text{수수료} = \text{상환금} \times \frac{\text{수수료율}}{100} \times \frac{\max(0, 36-m)}{36}$ (3년 경과 시 0원 전액 면제).
    - 중도상환 후 잔여 기간 잔액 감소에 따른 절약 이자 및 순 절감액 산출.
- **결과 대시보드 지표 및 시각화**:
  - **3대 상환방식 비교 카드**: 방식별 첫 달 상환액, 마지막 달 상환액, 총 대출이자, 총 상환금액 한눈에 대조.
  - **잔액 및 누적 상환 차트**: Recharts 기반 대출 잔액 감소 곡선 및 누적 원금/이자 상환 영역 차트.
  - **월별 상세 상환 스케줄표**: 회차별 원금, 이자, 월 상환액, 대출 잔액 상세 표기 및 CSV 다운로드 지원.

### 3.5 [금융/급여] 연봉 실수령액 계산기 (`SalaryApp` - 구현 완료)
- **기능 요약**: 2026년 대한민국 최신 4대 사회보험 요율 및 국세청 근로소득 간이세액표 누진세율을 기반으로, 세전 연봉/월급에서 공제되는 6대 항목을 차감한 실제 월/연 실수령액과 세부 공제 명세를 도출하는 계산기.
- **주요 입력 파라미터**:
  - 급여 기준: 연봉 / 월급 선택.
  - 세전 금액: 직접 금액 입력 (빠른 프리셋: `3,000만`, `4,000만`, `5,000만`, `6,000만`, `7,000만`, `1억` 및 `+100만`, `+500만`, `+1,000만` 증감 지원).
  - 퇴직금 지급 방식: 별도 지급(기본) / 연봉 포함(1/13 분할).
  - 비과세 식대: 기본 20만 원 (직접 수정 가능).
  - 인적공제: 부양가족 수(본인 포함 1~11인), 20세 이하 자녀 수(0~10인).
- **2026년 기준 4대 보험 및 세무 산출 공식**:
  - **국민연금 (4.5%)**: 기준소득월액 상한액(6,170,000원, 월 최대 277,650원), 하한액(390,000원, 월 17,550원). 원단위 절사.
  - **건강보험 (3.545%)**: 과세 대상 월 급여의 3.545%. 10원 미만 절사.
  - **노인장기요양보험 (건강보험료의 12.95%)**: 보수월액 기준 약 0.4590775%. 10원 미만 절사.
  - **고용보험 (0.9%)**: 과세 대상 월 급여의 0.9%. 10원 미만 절사.
  - **근로소득세 (국세청 간이세액표 누진세율)**:
    - 연간 총급여 환산 후 근로소득공제(구간별 5~70%) 차감.
    - 기본공제(인당 150만 원) 및 특별소득공제 차감 후 과세표준 8단계 누진세율(6%~45%) 적용.
    - 근로소득세액공제 및 자녀 세액공제 차감 후 월 근로소득세 도출 (10원 미만 절사).
  - **지방소득세 (10%)**: 산출된 월 근로소득세의 10% (10원 미만 절사).
- **결과 대시보드 지표 및 시각화**:
  - **메인 지표**: 월 예상 실수령액 및 연간 환산 합계, 세전 대비 실수령 비율(%).
  - **3단 서브 지표**: 세전 월 환산액, 월 총 공제액, 총 공제율(%).
  - **공제 비중 도넛 차트**: 실수령액 vs 4대 보험 vs 소득세/지방소득세 비율 시각화.
  - **6대 공제 세부 내역표**: 국민연금, 건강보험, 노인장기요양보험, 고용보험, 소득세, 지방소득세 항목별 월 공제액, 연간 납부액, 공제 비중 및 근로자/회사 부담금 전환 비교.

### 3.6 [생활/건강] BMI & 비만도 계산기 (`BmiApp` - 구현 완료)
- **기능 요약**: 신장과 체중을 기반으로 체질량지수(BMI)를 산출하고, 대한비만학회(KSSO) 한국인 공식 기준에 따른 비만도 판정, 적정 표준 체중 및 체중 조절 가이드를 스펙트럼 게이지와 카드로 제공.
- **주요 입력 파라미터**:
  - 신장: 100cm ~ 250cm (정밀 슬라이더 + `-1cm`/`+1cm` 미세 조절 + 빠른 프리셋: `160cm`, `165cm`, `170cm`, `175cm`, `180cm`).
  - 체중: 30kg ~ 200kg (정밀 슬라이더 + `-1kg`/`+1kg` 미세 조절 + 빠른 프리셋: `50kg`, `60kg`, `70kg`, `80kg`, `90kg`).
  - 성별: 남성 / 여성 (표준 체중 공식 분기 반영).
- **의학적 산출 로직 (`src/utils/bmiCalculator.ts`)**:
  - BMI 공식: $\text{BMI} = \text{체중(kg)} / (\text{신장(m)})^2$
  - 대한비만학회(KSSO) 한국인 6단계 판정 기준:
    - 저체중: BMI < 18.5
    - 정상: 18.5 ≤ BMI < 23.0
    ...
    - 3단계 고도비만: BMI ≥ 35.0
  - 적정 표준 체중: 남성 $\text{신장(m)}^2 \times 22$, 여성 $\text{신장(m)}^2 \times 21$
  - 정상 체중 범위: 하한 $\text{신장(m)}^2 \times 18.5$ ~ 상한 $\text{신장(m)}^2 \times 22.9$
- **결과 대시보드 지표 및 시각화**:
  - **메인 지표**: 현재 BMI 수치 및 6단계 판정 뱃지.
  - **3단 서브 지표**: 나의 적정 체중, 정상 체중 범위, 체중 조절 목표(감량/증량 권장량).
  - **비만도 스펙트럼 게이지**: 저체중부터 고도비만까지 6개 구간의 스펙트럼 컬러 바 위에 현재 위치를 표시하는 핀포인트 마커.

### 3.7 [금융/투자] 목표 자산 역산 계산기 (`GoalApp` - 구현 완료)
- **기능 요약**: "N년 뒤 목표 자산(예: 5억, 10억)을 모으려면 매월 얼마씩 저축/투자해야 할까?"를 재무 PMT 공식을 기반으로 역산하는 금융 계산기.
- **주요 입력 파라미터**:
  - 목표 자산 ($FV$): 1,000만 ~ 100억 원 (빠른 프리셋: `+5,000만`, `+1억`, `+3억`, `+5억`, `+10억`).
  - 목표 기간 ($n$): 1 ~ 40년 (빠른 프리셋: `3년`, `5년`, `7년`, `10년`, `15년`, `20년`, `30년`).
  - 연 예상 수익률 ($r$): 0% ~ 30%, step 0.5% (빠른 프리셋: `3.5% 예적금`, `5.0% 채권혼합`, `7.0% 글로벌주식`, `10.0% 적극투자`).
  - 현재 보유 초기 자금 ($PV$): 0원 ~ 목표 자산 (빠른 프리셋: `0원`, `+1,000만`, `+3,000만`, `+5,000만`, `+1억`).
  - 과세 방식:
    - `일반과세 (15.4%)`: 기본 이자소득세 14% + 지방소득세 1.4% 원천징수.
    - `ISA 절세 (9.9%)`: 개인종합자산관리계좌 비과세 한도 초과분 9.9% 분리과세.
    - `비과세 (0%)`: 청년도약계좌 등 이자소득세 0% 상품.
    - 선택 시 하단에 각 세제 혜택에 대한 친절한 설명 가이드 실시간 제공.
- **핵심 역산 로직 (Financial PMT & 월복리 공식)**:
  - 초기 목돈($PV$)의 미래 가치($FV_{PV} = PV \times (1 + i)^m$)를 차감한 순수 적립 목표액을 기준으로 월초 납입 기준 필요 월 적립액($PMT$) 역산.
  - 수익률 0% 및 저수익/고수익 전 구간에서 수학적 예외 없이 안정적으로 처리.
  - 선택된 과세 체계(15.4%, 9.9%, 0%)에 따른 세후 목표 달성 필요액 정확히 산출.
- **결과 대시보드 지표 및 시각화**:
  - **메인 지표**: 목표 달성에 필요한 **매월 적립액** (예: `월 234만 원`).
  - **3단 서브 지표**:
    - `총 투입 원금`: 초기 자금 + (월 적립액 × 개월 수)
    - `예상 복리 수익`: 목표 자산 - 총 투입 원금
    - `이자/수익 기여도`: 전체 목표 자산 중 복리 이자가 차지하는 비중 (%)
  - **자산 형성 궤적 차트**: 0년부터 목표 연차까지 [초기 자금 + 누적 적립 원금 + 누적 복리 수익]이 목표 금액으로 도달하는 누적 영역형 차트(`AreaChart`).
  - **수익률 시나리오 대조**: 예적금(연 3.5%), 인덱스 펀드(연 7%), 적극 투자(연 10%) 시 월 적립금 부담 비교.
...
- **[TODO] 계산기 최하단 추가 정보(상식 및 유의사항) 시각 디자인 고도화**:
  - 현재 단일 리스트 형태인 각 계산기 최하단 안내 패널(`InfoCard`)을 정보 성격(핵심 공식/개념, 세무·법적 유의사항, 실전 팁 등)에 따라 더욱 읽기 편하고 심미적인 형태(카드형 그리드, 탭 또는 아코디언 분할, 아이콘 및 시각 계층 강화)로 고도화.
...
7. **목표 자산 역산 계산기 (`GoalInfoCard.tsx`)**:
   - 시간의 힘 (복리 레버리지): 투자 기간이 5년에서 10년으로 2배 늘어나면 복리의 마법으로 인해 매월 넣어야 하는 적립금 부담은 3분의 1 수준으로 감소
   - 초기 시드머니의 위력: 초기에 얼마라도 목돈을 거치해두면 복리로 함께 불어나 전체 목표 달성에 필요한 월 적립 부담 완화
   - ISA 등 절세계좌 활용: 일반 계좌의 이자소득세(15.4%) 대신 ISA(9.9% 분리과세)나 비과세 혜택을 이용한 세후 자산 형성 가속화
   - 현실적인 기대 수익률 설정: 과도한 고수익 가정 지양, 예적금·채권·주식을 안배한 현실적인 목표(연 4~8%) 설정 권장 및 원금 손실 위험 안내

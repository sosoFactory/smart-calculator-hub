...
- **주요 기능 명세**:
  ...
  2. **입력 폼 표준화 (`GoalForm.tsx`)**:
      ...
      - 목표 기간 ($n$): 상단 실시간 수치 표기 + 풀 와이드 정밀 슬라이더(1~40년) + 7대 프리셋 (`3년`, `5년`, `7년`, `10년`, `15년`, `20년`, `30년`).
      - 연 예상 수익률 ($r$): 비현실적인 마이너스 범위를 제거하고 0%~30% 유효 범위로 제한. 상단 실시간 수치 표기 + 풀 와이드 정밀 슬라이더(step: 0.5%) + 4대 프리셋 (`3.5% 예적금`, `5.0% 채권혼합`, `7.0% 글로벌주식`, `10.0% 적극투자`).
      - 현재 보유 초기 자금 ($PV$): 직접 숫자 입력(0원 ~ 목표 자산) + 한글 금액 라벨 + 5대 프리셋 (`0원`, `+1,000만`, `+3,000만`, `+5,000만`, `+1억`).
      - 과세 유형 반응형 최적화 및 ISA 친절 안내:
        - `SegmentedControl` 형태를 해상도와 무관하게 일관된 단일 3분할 탭 형태로 유지.
        - **반응형 텍스트 쇼/하이드**: 1024px(좌측 폼 열 약 290px) 및 컨테이너 폭에 맞춰 텍스트가 넘치거나 줄바꿈되지 않도록 라벨/세율 표시를 유동 최적화:
          - 상단 실시간 세율: `whitespace-nowrap`을 적용하고 좁은 폭에서는 `일반과세 (15.4%)` 대신 `(15.4%)`로 간결화 (`<span className="hidden sm:inline">일반과세 </span><span>(15.4%)</span>`).
          - 세그먼트 버튼 라벨: 컴팩트하게 `일반`, `ISA`, `비과세`를 기본 표기하고, 가용 폭이 넓을 때만(`xl:inline`) 세율 괄호(` (15.4%)`, ` (9.9%)`, ` (0%)`)를 노출하여 텍스트 찌그러짐 원천 방지.
        - **ISA 및 과세 방식 친절 안내 가이드**: ISA(개인종합자산관리계좌)를 생소해하는 사용자를 위해 컨트롤 하단에 선택된 항목별 알기 쉬운 한 줄 설명 제공:
          - 일반과세: 금융상품 이자·배당 수익에 기본 부과되는 이자소득세(14%) 및 지방소득세(1.4%) 원천징수 안내.
          - ISA 절세: 200만~400만원 비과세 한도 및 초과 수익 9.9% 분리과세 혜택 안내.
          - 비과세: 청년도약계좌 등 이자소득세 0% 상품 안내.
  ...
  7. **1024px 태블릿 및 모바일 반응형 무결점 레이아웃 (`GoalApp.tsx` & 컴포넌트)**:
      - **플랫폼 공통 2컬럼 레이아웃 통일 (`GoalApp.tsx`)**: 다른 모든 계산기(`LoanApp`, `SalaryApp`, `CompoundInterestApp`, `BmiApp`)와 동일하게 1024px(`lg`)부터 일관된 2컬럼 레이아웃(`lg:grid-cols-12 gap-4 sm:gap-6`, 좌측 폼 5 / 우측 결과 7) 유지.
      - **우측 요약 및 대조 카드 유동 리플로우 (`GoalSummaryCards`, `GoalRateComparisonCard`, `GoalInfoCard`)**:
        - 1024px 태블릿/랩탑(우측 콘텐츠 폭 약 420px)에서 카드가 130px로 찌그러지는 현상을 원천 방지하기 위해 `grid-cols-[repeat(auto-fit,minmax(185px,1fr))]` 적용 (1024px에서는 2열로 210px씩 넉넉히 리플로우, 1280px 이상에서는 3열 한 줄 정렬, 모바일에서는 1열).
      - **차트 및 폼 모바일 가독성**:
        - `GoalChartCard`: 상단 범례에 `flex-wrap`을 적용하여 360px 모바일 화면에서도 잘림 없는 줄바꿈 보장.
        - `GoalForm`: 헤더 텍스트 영역 `min-w-0` 및 초기화 버튼 `shrink-0` 적용, 슬라이더와 숫자 입력창 가로 결합(`flex items-center gap-3`)으로 세로 길이 최적화.

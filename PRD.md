...
- **수치 및 금액 입력 필드 (`NumericInput`)**:
  - 터치 친화적 대형 규격, 우측 정렬, 볼드 폰트 적용.
  - 통화/수치는 3자리 콤마 포맷팅 자동 적용, 인풋 우측 내부에 단위 심볼(`원`, `%`, `년`, `cm` 등) 표준 배치.
  - 라벨 상단에는 현재 설정값만 간결히 노출하여 중복 표기 방지.
- **슬라이더 & 프리셋 칩 (`Slider` & `SelectableChip`)**:
  - 슬라이더는 인풋과 무리하게 한 줄에 결합하지 않고 전폭 단독 배치하여 터치/드래그 조작성 확보.
  - 슬라이더 하단에는 자주 쓰는 대표값을 선택할 수 있는 프리셋 칩을 균형 있게 배치.
- **옵션 선택 세그먼트 (`SegmentedControl`)**:
  - 뷰포트에 따라 드롭다운으로 변경하지 않고 단일 3분할 탭 형태를 일관되게 유지.
  - 화면 폭이 좁을 때는 핵심 명칭만 노출하고, 가용 폭이 넓을 때 상세 설명을 병기하는 반응형 텍스트 제어 적용.

#### 2.3.4 결과 대시보드 표준 규격 (Result Dashboard Standards)
- **메인 핵심 결과 카드 (Primary Result Card)**:
  - 결과 대시보드 최상단에 배치하여 계산기의 최종 핵심 지표를 대형 수치와 일렉트릭 라임 액센트로 강조.
  - 한글 금액 단위(예: `1억 5,000만 원`, `월 234만 원`) 병기.
  - 계산 결과를 한 번에 클립보드에 담을 수 있는 원클릭 복사 버튼 및 시각적 피드백 제공.
- **서브 요약 지표 카드 유동 리플로우 (Sub Metric Cards)**:
  - 핵심 지표 하단에 배치되는 3단 보조 지표 카드는 컨테이너 너비에 따라 유동 리플로우 적용.
...
- **금융 및 생활 상식 안내 카드 (`InfoCard`)**:
  - 모든 계산기 최하단에 배치하여 해당 계산기와 관련된 핵심 공식, 세무 유의사항, 실전 팁을 2~4개 블록으로 압축 제공.
  - 컬러 이모지를 배제하고 단정한 텍스트 및 Lucide 라인 아이콘으로 신뢰감 형성.
